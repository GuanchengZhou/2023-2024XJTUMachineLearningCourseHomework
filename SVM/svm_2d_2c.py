...
if __name__=='__main__':
    ...
    for i, model in enumerate(models):
        ...
        plt.plot(train_x[:,0], -p1/p2*train_x[:,0]-b/p2, c=colors[i], label='C={}'.format(Cs[i]))
        pred_y = model.predict(test_x)
        pr = np.sum(pred_y==test_y) / len(pred_y) * 100
        pred_y2 = model.predict(train_x)
        pr2 = np.sum(pred_y2 == train_y) / len(pred_y2) * 100
        print(Cs[i],'&',pr2,'&',pr)
